function [lastCentroid,centStamp]=optoMatchCentroids2Wells(cenDat,centers,speedThresh,distanceThresh,lastCentroid,centStamp,tElapsed)
...
% Returns minimum distance to each previous centroid and the indices (j)
% Of the temp centroid with that distance
[lastCenDistance,j]=min(g);

% For the centroids j, calculate speed and distance to ROI center for
% thresholding (do nothing if no centroids detected);
