...
bins=linspace(0,8,500);
tmp_speed=speed(:);
tmp_speed(tmp_speed==0)=[];                % Remove trials where flies aren't moving
speed_hist=histc(tmp_speed,bins);          % generate speed histogram
speed_hist=speed_hist./sum(speed_hist);
speed_cdf=cumsum(speed_hist);              % convert to cummulative distribution to find upper bound
[v,cut_off]=min(abs(0.95-speed_cdf));      % find a conservative cut off value that captures 95% of of the frames
speed_thresh=bins(cut_off);
clearvars tmp_speed speed_hist speed_cdf cut_off bins

valid_trials=speed>0 & speed<speed_thresh;
%}
...
%x=x(mod(1:size(x,1),3)==0,:);
y=cenDat(mod(1:size(cenDat,1),2)==0,:);
%y=y(mod(1:size(y,1),3)==0,:);
clearvars cenDat 
